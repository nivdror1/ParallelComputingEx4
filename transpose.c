...
void readMatrix(int** m, char* relPath, int size){
    ...
    for(int p=0; p<size; p++){
        ...
        for (int b=0 ; b<(BS*BS); b++){
            errW = fread(&i, sizeof(int),1,file);
            errW = fread(&j, sizeof(int),1,file);
            errW = fread(&val, sizeof(int),1,file);
            m[i][j] = val;
        }
...
void saveInput(int** matrix, int rank, int size){
    ...
    for(int i=0; i<BS; i++){
        ...
        for(int j=0; j<BS;j++){
            ...
            errW = fwrite(&globe_i, sizeof(int),1,file);
            errW = fwrite(&globe_j, sizeof(int),1,file);
            errW = fwrite(&matrix[i][j], sizeof(int),1,file);
        }
    }
...
//	printf("test\n");
	...
	for(int i=0; i<BS; i++){
		for(int j=0; j<BS;j++){
            ...
            errW = fwrite(&globe_j, sizeof(int),1,file);
            errW = fwrite(&globe_i, sizeof(int),1,file);
            errW = fwrite(&A[i][j], sizeof(int),1,file);
		}
	}
    int errC = fclose(file);
